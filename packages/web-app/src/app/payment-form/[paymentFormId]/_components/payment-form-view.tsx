...
import { ColorScheme, FontFamily, PaymentForm, Product } from '@/schemas/payment-form';
...
export function PaymentFormView({ paymentForm }: { paymentForm: PaymentForm }) {
    const paymentFormId = paymentForm.paymentFormId || (paymentForm as { id?: string }).id || '';
    ...
    // Compute totalAmount dynamically during render
    const totalAmount = selectedProducts.reduce(
        (sum, product) => sum + product.productPrice,
        0
    );

    // Parse pre-selected products from query parameters upon load
    useEffect(() => {
        if (typeof window !== 'undefined' && paymentForm.paymentFormProducts && paymentForm.paymentFormProducts.length > 0) {
            const params = new URLSearchParams(window.location.search);
            const productsParam = params.get('products');
            if (productsParam) {
                const productNames = productsParam.split(',').map(name => decodeURIComponent(name).trim());
                const matchingProducts = paymentForm.paymentFormProducts.filter((p: Product) =>
    ...
    }, [paymentForm.paymentFormProducts]);
